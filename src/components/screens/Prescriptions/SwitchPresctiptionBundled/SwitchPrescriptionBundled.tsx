...
import {
  switchToSemaglutideBundledEvent,
...
const SwitchPrescriptionBundled = () => {
  ...
  console.log('Active subscriptions:', patientSubscriptions);

  const tirzepatideBundleSubscription = patientSubscriptions?.find(
    sub => sub.price === 449
  );
  console.log('Tirz bundle sub:', tirzepatideBundleSubscription);
  ...
  const handleFullRefund = async (orderId: any) => {
    await axios.post(
      '/api/stripe/utils/charge/tirzepatide-refund-issued-bundle',
      {
        orderId,
  ...
  const handleSwitchToSemaglutide = async (orderId: any) => {
    ...
    try {
      await axios.post(
        '/api/stripe/utils/charge/switched-to-semaglutide-bundled',
        {
          orderId,
          patientId: patientInfo?.id,
      ...
      await switchToSemaglutideBundledEvent(
        patientInfo?.profiles?.email || '',
        patientInfo?.profiles?.first_name || '',
    ...
    <Container>
      <>
        ...
        <Typography variant="body1">
          {`We can offer you the equivalent dosage of semaglutide. Since you ordered a 1 month supply of tirzepatide (${
            tirzepatideDosage?.[1]
          } mg vial), you would receive a 1 month supply of semaglutide (${dosageConversions} mg vial). If you choose this, we will ship your order within 2 business days and you should receive your order within 4 business days. Because we know this is an inconvenience and we want to help you achieve lasting weight loss affordably, we will offer a 35% refund off your order immediately, which typically takes 5-7 business days to be returned to your bank account. Because you paid $${tirzepatideOrders?.[0]?.amount_paid?.toFixed(
            2
        ...
        <Typography variant="body1" sx={{ marginBottom: '16px' }}>
          {`Alternatively, you can choose to cancel your order entirely and you will receive a full refund on your medication. The full refund would be processed immediately and typically takes 5-7 business days to be returned to your bank account. You paid $${tirzepatideOrders?.[0]?.amount_paid?.toFixed(
            2
          )} for your medication, so you will receive that amount as a refund if you cancel your order. However, you will not receive any medication if you choose the full refund.`}
        </Typography>
        <Box
          sx={{
            display: 'flex',
...
export default SwitchPrescriptionBundled;
