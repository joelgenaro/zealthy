...
interface WeightLossBulkYearAddOnProps {
  onNext: (s?: string) => void;
  variant?: string;
}

const WeightLossBulkYearAddOn = ({ onNext }: WeightLossBulkYearAddOnProps) => {
  ...
  const checkoutHeader =
    medications?.[0]?.name === 'Semaglutide'
      ? 'Your discount for a year of Semaglutide has been applied. You won’t be charged for either unless your provider approves your Rx request & we’re ready to ship it to you.'
      : `Your discount for a year of Tirzepatide and the next ${months} months of your membership has been applied. If you are not eligible for medication, you will be refunded.`;
    ...
    async (newSubscription?: any) => {
      ...
      if (vwoClient && patientInfo?.id) {
        await Promise.allSettled([
          vwoClient.track('4313', 'bundledYearPlanPurchased', patientInfo),
          vwoClient.track(
            'Clone_Clone_4313',
            'bundledYearPlanPurchased',
            patientInfo
          ),
          vwoClient.track('9057_1', 'bundledYearPlanPurchased', patientInfo),
          vwoClient.track('9057_2', 'bundledYearPlanPurchased', patientInfo),
          vwoClient.track('9057_3', 'bundledYearPlanPurchased', patientInfo),
        ]);
      }
  ...
  const handlePrescriptionRequest = async () => {
    ...
    const metadata = {
      ...
      reason: `weight-loss-bulk`,
      zealthy_patient_id: patientInfo.id,
      zealthy_product_name: `Weight Loss ${medications?.[0]?.name} Bundled`,
    };
    ...
    if (price > 0) {
      const { data, error } = await createInvoicePayment(
        ...
        price * 100,
        metadata,
        '12 Months Weight Loss Semaglutide Membership',
        false,
        uniqueKey
      );
      ...
      {page === 'confirm' && (
        <>
            ...
            {!bulkCoaching && !existingBulkSubscription && (
              <Box sx={{ marginBottom: '16px' }}>
                ...
                <Typography variant="body1">
                  ...
                  >
                    ${medications?.[0]?.name === 'Semaglutide' ? '297' : '449'}
                  </Typography>
                  {`$${
                    medications?.[0]?.name === 'Semaglutide' ? '237' : '359'
                  } per month`}
                </Typography>
                <Typography variant="body1">
          ...
          <Typography
            ...
            sx={{ color: '#008A2E' }}
          >
            {medicationName?.toLowerCase().includes('tirzepatide')
          ...
          <Box sx={{ textAlign: 'center' }}>
            ...
            >
              ...
              medications?.[0]?.price !== 1969 &&
              medications?.[0]?.price !== 3267
                ? 'Pay $0 today'
                : `Confirm order - ${
                    !existingBulkSubscription ||
                    weightLossSubscription?.status === 'canceled' ||
            ...
            >
              {`*This is what Zealthy expects to last 12 months. If you are determined not to be eligible by your provider, you will be able to get a refund. If you choose to pay with Klarna, you can split this purchase into equal installments of $${Math.round(
                price / 7
              )}-$${Math.round(
                price / 4
              )} typically paid monthly, which you will pay across equal installments every 30 days or every 2 weeks until you have paid the $${price} in full. This includes the next 2 months of your membership and the total cost of the 11 month supply of medication.`}
            </Typography>
            <Typography
              fontSize="0.75rem !important"
              fontStyle="italic"
...
export default WeightLossBulkYearAddOn;
