...
const RateGoogleModal = ({ isOpen, onClose, addToQueue }: Props) => {
  const { data: notifications } = useFutureNotifications('RATE_GOOGLE');
  ...
  const handleRate = async () => {
    try {
      ...
      if (isMobileDevice) {
        return setTimeout(() => {
          window.open('https://g.page/r/CU4xrOjIDDQmEAE/review', '_top');
        });
      }
      return window.open('https://g.page/r/CU4xrOjIDDQmEAE/review', '_blank');
    } catch (e: any) {
  ...
  const handleSkip = async () => {
    try {
      ...
      throw new Error('Error skipping google rating modal');
    } finally {
      onClose();
    }
  };
    ...
    <Modal open={isOpen}>
      ...
      >
        <Typography variant="h2">
          Please help with a public review on Google!
        </Typography>
        <Typography>
          {
...
export default RateGoogleModal;
