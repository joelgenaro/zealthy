...
export function WeightLossSixMonth({
  videoUrl,
  onNext,
  oneMonthPrice,
}: ConfirmationProps) {
  ...
  const existingBulkSubscription = useMemo(() => {
    return (
      daysToPeriodEnd > 150 && weightLossSubscription?.status !== 'canceled'
    );
  ...
  const onSuccess = useCallback(
    async (newSubscription?: any) => {
      ...
      window.VWO?.event('6MonthPrescriptionRequestSubmitted');

      await Promise.allSettled([
        vwoClient?.track(
          '8685',
          '6MonthPrescriptionRequestSubmitted',
          patientInfo
        ),
        vwoClient?.track(
          '9057_1',
        ...
        vwoClient?.track('6315', '6MonthGlp1PaidFor', patientInfo),
        vwoClient?.track('9363', '6MonthGlp1PaidFor', patientInfo),
        vwoClient?.track('9205', '6MonthGlp1PaidFor', patientInfo),
      ]);
  ...
  let medicationSupplyText = '{0} 6 month supply';
  ...
  if (language === 'esp') {
    ...
    chargedIfPrescribedText =
      '*Se le cobrará ${0} si se le receta {1} compuesto. Esto incluye los próximos 2 meses de su membresía y el costo total del suministro de medicamento para 3 meses. Esto es lo que Zealthy espera que dure 3 meses. Su proveedor puede recomendar una cantidad de dosis diferente, lo que cambiaría el precio. No será responsable de ninguna parte de esta cantidad si no se le receta.';
        ...
        <>
          ...
          >
            {variation9502?.variation_name === 'Variation-2'
              ? ''
              : `Your discount for 6 months of ${medicationName} weekly injections and the next 5 months of your membership has been applied. You won’t be charged for either unless your provider approves your Rx request & we’re ready to ship it to you.`}
          </Typography>
              ...
              <Stack gap={1.5} fontStyle="italic" textAlign="left">
                <Typography variant="subtitle2" fontSize="0.75rem !important">
                  {chargedIfPrescribedTextKlarna
                    ...
                    .replace('{3}', `${Math.round(prePayTotal / 4)}`)}
                </Typography>
