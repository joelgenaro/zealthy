...
const FHL_DISPLAY_NAME = 'Hair Loss Medication';
const HL_DISPLAY_NAME = 'Female Hair Loss Medication';

const FemaleHairLossRenewal = () => {
  ...
  useEffect(() => {
    const startFlow = async () => {
      const previousPrescription = await supabaseClient
        ...
        .in('medication_quantity.medication_dosage.medication.display_name', [
          FHL_DISPLAY_NAME,
          HL_DISPLAY_NAME,
        ])
        .eq('status', 'expired')
      ...
      setFlow('fhl-prescription-renewal');
      addMedication({
        display_name: 'Female Hair Loss Renewal',
        dosage:
  ...
  useEffect(() => {
    ...
    async function createVisit() {
      ...
      addQuestionnaires(
        mapCareToQuestionnaires(['Female Hair Loss Prescription Renewal'])
      );
    }
    createVisit().then(() => {
      router.push(
        '/patient-portal/questionnaires-v2/hair-loss-female-prescription-renewal'
      );
    });
...
FemaleHairLossRenewal.getLayout = (page: ReactElement) => (
  <PatientPortalNav>{page}</PatientPortalNav>
);

export default FemaleHairLossRenewal;
