...
export const finalizeInvoice = async (invoice: Stripe.Invoice) => {
  try {
    ...
    if (patientId) {
      ...
      console.log(`Finalize invoice ${invoice.id}`, {
        meta: {
          invoice: invoice.id,
          patient: patientId,
          nextPaymentAttempt,
      ...
      return supabaseAdmin.from('invoice').upsert(updateObj).throwOnError();
    } else {
      throw new Error(`Could not find patient for ${invoice.customer}`);
    }
  } catch (err: any) {
    console.error('finalizedInvoice-Err', err);
    throw new Error(
      `Error in finalizeInvoice: ${
        typeof err === 'string'
          ? err
