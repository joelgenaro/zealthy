import { paymentSuccessMissedVisitFee } from '@/utils/freshpaint/events';
import Stripe from 'stripe';
import getStripeInstance from '../../../createClient';
import { getServiceSupabase } from '@/utils/supabase';

export const processMissedVisit = async (invoice: Stripe.Invoice) => {
  ...
  if (invoice.charge) {
    ...
    return paymentSuccessMissedVisitFee(
      profileId,
      email,
      paymentSucceededAt,
      total,
    ...
    console.log({
      message: `Could not find a charge for invoice: ${invoice.id}`,
    });
  }
};
