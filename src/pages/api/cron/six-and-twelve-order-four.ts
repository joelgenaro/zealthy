...
export default async function handler(
  ...
  const twoHundredSixtyDaysAgo = addDays(new Date(), -260).toISOString();
  const twoHundredSeventyDaysAgo = addDays(new Date(), -270).toISOString();

  const futureOrders4 = await supabase
    .from('order')
    .select('*, prescription(*)')
    .eq('order_status', 'FUTURE_ORDER_4')
    .limit(1)
    .lt('created_at', twoHundredSixtyDaysAgo)
    .gt('created_at', twoHundredSeventyDaysAgo)
    .then(data => data.data);

  console.log(twoHundredSixtyDaysAgo, twoHundredSeventyDaysAgo);
  console.log(futureOrders4);

  const groupIds = new Set(futureOrders4?.map(order => order.group_id));
  await Promise.all(
