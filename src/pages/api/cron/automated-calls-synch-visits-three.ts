...
export default async function handler(
  ...
  // Appointments that start between 3.5 hours and 4 hours in the future
  const fourHours = addHours(new Date(), 4);
  const startOfRange1 = subMinutes(fourHours, 30).toISOString();
  ...
  const futureAppointment = await supabaseAdmin
    ...
    .lt('starts_at', fourHours.toISOString())
    .or(
