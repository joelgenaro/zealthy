...
export default async function handler(
  ...
  // Appointments that start between 23.5 hours and 24 hours in the future
  const twentyFourHours = addDays(new Date(), 1);
  const startOfRange1 = subMinutes(twentyFourHours, 30).toISOString();
  ...
  const futureAppointment = await supabaseAdmin
    ...
    .lt('starts_at', twentyFourHours.toISOString())
    .or(
