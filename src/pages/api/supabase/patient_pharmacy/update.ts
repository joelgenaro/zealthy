...
type UpdatePayload = {
  ...
  old_record: PatientPharmacy;
};

const handlePatientPharmacyUpdate = async (
  req: NextApiRequest,
  res: NextApiResponse
) => {
  try {
    ...
    if (patient.dosespot_patient_id) {
      console.log({
        message: `Update => Adding Preferred Pharmacy to dosespot for patient ${record.patient_id}`,
      });
...
export default async function UpdateOrder(
  ...
  return supabaseWebhookHandlerWrapper(req, res, handlePatientPharmacyUpdate);
}
