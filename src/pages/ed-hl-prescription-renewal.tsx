...
const EDHL_DISPLAY_NAME = 'EDHL Medication';
...
const EDHLRenewal = () => {
  ...
  useEffect(() => {
    const startFlow = async () => {
      ...
      setFlow('edhl-prescription-renewal');
      addMedication({
        display_name: previousPrescription?.medication || 'Sex + Hair Renewal',
        dosage:
        ...
        name: previousPrescription?.medication || 'Sex + Hair Renewal',
  ...
  useEffect(() => {
    ...
    async function createVisit() {
      ...
      addQuestionnaires(mapCareToQuestionnaires(['EDHL Prescription Renewal']));
    }
    createVisit().then(() => {
      router.push(
        '/patient-portal/questionnaires-v2/ed-hl-prescription-renewal'
      );
    });
...
EDHLRenewal.getLayout = (page: ReactElement) => (
  <PatientPortalNav>{page}</PatientPortalNav>
);

export default EDHLRenewal;
